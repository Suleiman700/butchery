...
class FatalErrors
{
    ...
    private function returnErrorMessage()
    {
        ...
        if (CONST_FATAL_ERRORS['EMAIL_ADMINS_ON_FATAL']['ENABLED']) {
            // Send email to admins in production environment or dev environment based on settings
            if ((CONST_FATAL_ERRORS['EMAIL_ADMINS_ON_FATAL']['SEND_IN_PROD_ENV'] && $CONSTANTS['IS_PROD']) || (CONST_FATAL_ERRORS['EMAIL_ADMINS_ON_FATAL']['SEND_IN_DEV_ENV'] && !$CONSTANTS['IS_PROD'])) {
                $adminEmail = CONST_FATAL_ERRORS['EMAIL_ADMINS_ON_FATAL']['EMAILS'][0];
                $message = "Fatal Error\n\nTrace ID: $traceID";
                // send to admin...
            }
        }
        ...
        $res = [
            'errors' => [
                [
                    'message' => 'An error occurred while processing your request.<br><br>Please contact support team and provide the following code:<br>',
                    'errorCode' => $traceID
                ]
            ]
        ];
